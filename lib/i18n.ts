...
const tr = {
  ...
  results: {
    heading: "Sonuçlar",
    note: "Oynanan maçların içi: goller, kartlar, kaçan penaltılar ve iki takımın kadrosu. Skorlar oyunun verisinden, maçın içi FotMob maç sayfalarından.",
    viewLabel: "Görünüm",
    fixtures: "Maçlar",
    table: "Puan durumu",
    matchdayLabel: "Hafta",
    allMatchdays: "Tüm haftalar",
    matchday: (md: number) => `${md}. hafta`,
    currentMark: "bu hafta",
    goCurrent: (md: number) => `${md}. haftaya dön`,
    teamLabel: "Takım",
    allTeams: "Tüm takımlar",
    showing: (md: string, team: string, n: number) => `${md}, ${team}: ${n} maç.`,
    empty: "Bu seçimde maç yok.",
    notPlayed: "oynanmadı",
    allPlayed: "tamamlandı",
    somePlayed: (played: number, total: number) => `${total} maçın ${played}'i oynandı`,
    open: "ayrıntı için dokun",
    noGoals: "Gol yok.",
    noEvents: "Bu maçın olay dökümü yok.",
    assist: "asist",
    ...
    cards: {
      Yellow: "sarı kart",
      Red: "kırmızı kart",
      YellowRed: "ikinci sarıdan kırmızı",
    },
    potm: "Maçın adamı",
    subs: "Oyuna girenler",
    lineupsMissing: "Kadro bilgisi yok.",
    source: (src: string, date: string) => `Maç içi verisi: ${src}, ${date}.`,
  },
  sourcesTable: {
    heading: "Kaynaklar ve tazelik",
    caption:
      "Her veri grubu ayrı çekiliyor ve biri bayat kalabilir. Aşağıdaki tarih, o grubun son başarılı güncellemesi.",
    colWhat: "Veri",
    colSource: "Kaynak",
    ...
    never: "hiç çekilmedi",
    manual: "elle",
    fixtures: "Fikstür ve skorlar",
    fixturesScope: "34 hafta, 306 maç",
    opta: "Opta güç sırası",
    optaScope: "18 kulüp",
    value: "Kadro değeri",
    valueScope: "18 kulüp",
    last: "Geçen sezon sırası",
    lastScope: "2025/26 nihai tablo",
    players: "Oyuncu listesi ve fiyatlar",
    playersScope: (gw: number, n: number) => `${n} oyuncu, ${gw}. hafta`,
    history: "Fiyat ve seçilme günlüğü",
    historyScope: (days: number, n: number) => `${days} gün, ${n} oyuncu`,
    lineups: "Son maç kadroları",
    lineupsScope: (teams: number, perTeam: number) => `${teams} takım, takım başına ${perTeam} maç`,
    results: "Maç içi (gol, kart, kadro)",
    resultsScope: (matches: number) => `${matches} maç`,
    predicted: "Tahmini 11",
    predictedScope: (n: number, gw: number) => `${n} önizleme, ${gw}. hafta`,
    // Tazelik uyarıları; taze grupta hiçbir şey yazılmıyor.
    stale: (days: number) => `${days} gündür eski`,
    behind: (feed: number, next: number) =>
      `besleme ${feed}. haftada, planlanan ${next}. hafta`,
    missing: (gw: number) => `${gw}. haftanınki yok`,
    staleCount: (n: number) => `${n} kaynak eski`,
  },
...
const en: Strings = {
  ...
  results: {
    heading: "Results",
    note: "What happened inside each match: goals, cards, missed penalties and both line-ups. Scores come from the game's own data, the match detail from FotMob match pages.",
    viewLabel: "View",
    fixtures: "Matches",
    table: "Table",
    matchdayLabel: "Matchweek",
    allMatchdays: "All matchweeks",
    matchday: (md: number) => `Matchweek ${md}`,
    currentMark: "current",
    goCurrent: (md: number) => `Back to matchweek ${md}`,
    teamLabel: "Team",
    allTeams: "All teams",
    showing: (md: string, team: string, n: number) => `${md}, ${team}: ${n} matches.`,
    empty: "No matches for this selection.",
    notPlayed: "not played",
    allPlayed: "complete",
    somePlayed: (played: number, total: number) => `${played} of ${total} played`,
    open: "tap for detail",
    noGoals: "No goals.",
    noEvents: "No match detail for this game.",
    assist: "assist",
    ownGoal: "own goal",
    penalty: "penalty",
    freeKick: "free kick",
    missedPenalty: "missed a penalty",
    cards: {
      Yellow: "yellow card",
      Red: "red card",
    ...
    subs: "Substitutes used",
    lineupsMissing: "No line-up data.",
    source: (src: string, date: string) => `Match detail: ${src}, ${date}.`,
  },
  sourcesTable: {
    heading: "Sources and freshness",
    caption:
      "Each data group is fetched separately and one can go stale. The date below is that group's last successful update.",
    colWhat: "Data",
    colSource: "Source",
    ...
    never: "never fetched",
    manual: "manual",
    fixtures: "Fixtures and scores",
    fixturesScope: "34 matchweeks, 306 matches",
    opta: "Opta power ranking",
    optaScope: "18 clubs",
    value: "Squad value",
    valueScope: "18 clubs",
    last: "Last season's table",
    lastScope: "2025/26 final table",
    players: "Player list and prices",
    playersScope: (gw: number, n: number) => `${n} players, matchweek ${gw}`,
    history: "Price and ownership log",
    historyScope: (days: number, n: number) => `${days} days, ${n} players`,
    lineups: "Recent line-ups",
    lineupsScope: (teams: number, perTeam: number) => `${teams} teams, ${perTeam} matches each`,
    results: "Match detail (goals, cards, line-ups)",
    resultsScope: (matches: number) => `${matches} matches`,
    predicted: "Predicted XI",
    predictedScope: (n: number, gw: number) => `${n} previews, matchweek ${gw}`,
    stale: (days: number) => `${days} days old`,
    behind: (feed: number, next: number) =>
      `feed on matchweek ${feed}, planning ${next}`,
    missing: (gw: number) => `none for matchweek ${gw}`,
    staleCount: (n: number) => `${n} sources stale`,
  },
  docTitle: (season: string) => `TFF FL Planner ${season}`,
  header: {
    title: "TFF FL Planner",
