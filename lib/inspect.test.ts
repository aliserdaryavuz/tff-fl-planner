...
describe.skipIf(!process.env.INSPECT)("gözlem", () => {
  it("haftanın kadrosu", () => {
    ...
    for (const [i, r] of rows.slice(0, 15).entries()) {
      console.log(
        `${String(i + 1).padStart(2)} ${r.player.name.padEnd(18)} ${byId[r.player.team].name.padEnd(15)} ${r.player.pos} ${String(r.player.price).padStart(5)}M  skor ${r.score.toFixed(2)}  start ${(r.startProb * 100).toFixed(0)}%`,
      );
    }
    const squad = buildSquad(rows).best!;
    console.log(`\n--- kadro (${formationLabel(squad.formation)}) ---`);
    console.log(
      `skor ${squad.xiScore.toFixed(1)} | fiyat ${squad.price} M | ilk11 ${squad.xiPrice} M | kaptan ${squad.captain?.name}`,
    );
