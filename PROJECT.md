...
| Sakat/cezalı | FotMob kadro sayfaları | `fetch-squads.mjs` | Oyunun API'sinde sakatlık yok; oyun listesi korunur, yalnız `fotmobId` + `status` eklenir. Eşleme tek yönlü ve üç aşamalı: tam ad (`fullName`), görünen kısa ad, en sonda forma numarası — forma yalnız ad da örtüşüyorsa sayılır (bkz. PLAN.md Faz 0) |
