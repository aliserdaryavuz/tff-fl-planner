  ...
  const card = (p: Player, small = false) => {
    ...
    return (
      ...
      >
        {isCaptain || isVice ? (
          <span
            className="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full font-cond text-micro font-bold"
            style={{
        ...
        <b className="mt-1 block truncate text-micro leading-tight font-semibold desk:text-caption">
          <PlayerLink player={p} />
          {p.status === "D" ? (
        ...
        <span className="mt-0.5 flex items-baseline justify-center gap-1 text-micro leading-tight tabular-nums desk:text-caption">
    ...
    <div>
      ...
      >
        ...
        <div
          ...
          style={{ borderColor: "var(--color-pitch-line)" }}
        />
        <span className="absolute top-2 left-3 rounded bg-black/40 px-1.5 py-0.5 font-cond text-caption font-semibold text-white">
      ...
      <div className="mt-2 rounded-xl border border-line bg-surface p-2">
        <div className="mb-1.5 text-caption font-semibold text-muted">{t.squad.bench}</div>
