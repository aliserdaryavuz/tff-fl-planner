...
export function GameweekBar({
  ...
  return (
    <section aria-labelledby="gw-heading" className="rounded-xl border border-accent/60 bg-surface p-3">
      <h2 id="gw-heading" className="mb-2 font-cond text-xl font-semibold tracking-wide">
        {t.gameweek.heading}
      </h2>
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          className="min-h-11 min-w-0 flex-1 rounded-[10px] border border-line bg-ground px-3 font-cond text-[22px] font-semibold"
        >
      ...
      <div className="mt-3 grid gap-2 desk:grid-cols-2 desk:gap-x-4">
        <div className="grid grid-cols-[1fr_auto] items-center gap-x-2.5">
          ...
          />
          <p className="col-span-2 -mt-0.5 text-xs text-muted">{t.gameweek.horizon.note}</p>
        </div>

        <div
          className={`grid grid-cols-[1fr_auto] items-center gap-x-2.5 ${horizon <= 1 ? "opacity-50" : ""}`}
        >
          <label htmlFor="decay" className="text-sm">
          ...
          <input
            ...
            className="col-span-2 w-full accent-accent"
          />
          <p className="col-span-2 -mt-0.5 text-xs text-muted">{t.gameweek.decay.note}</p>
          {window.length > 1 ? (
            <div
            ...
            >
              {window.map(({ md, w }) => (
                ...
                >
                  ...
                  <b className="mt-0.5 block font-cond text-[13px] font-bold tabular-nums">
