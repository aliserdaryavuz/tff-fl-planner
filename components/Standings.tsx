...
export function Standings({
  ...
  const head = "text-right text-xs text-muted";
  ...
  return (
    <details className="group border-t border-line pt-2">
      ...
      <p className="mt-1.5 text-[13px] text-muted">{t.standings.note}</p>
        ...
        {table.map((r) => {
          ...
          return (
            <button
              ...
              className={[
                "grid min-h-10 w-full grid-cols-[22px_minmax(0,1fr)_28px_28px_28px_28px_36px_36px] items-center gap-x-1.5 border-b border-line border-l-[3px] px-1 text-left text-[13px] tabular-nums",
                on ? "border-l-accent bg-accent/10" : "border-l-transparent hover:bg-white/3",
              ].join(" ")}
            >
              <span className="text-right font-cond text-[15px] font-semibold text-muted">{r.rank}</span>
              ...
              <span className="text-right font-cond text-[15px] font-bold">{r.pts}</span>
            </button>
          );
        })}
      </div>
